...
export const generateRadixColors = ({
	...
	accentScaleColors[9] = getButtonHoverColor(accent9Color, [
		accentScaleColors,
	])
...
function blendAlpha(
	...
	if (round) {
		return (
			Math.round(background * (1 - alpha)) +
			Math.round(foreground * alpha)
		)
	}
...
function getAlphaColorSrgb(
	...
	return formatHex(
		new Color("srgb", [r, g, b], a).toString({ format: "hex" }),
	)
}
